...
| 步骤 | 名称 | 状态 | 测试 | 完成日期 |
|------|------|------|------|----------|
| Step 1 | Proposal 校验与确认创建工作流测试基线 | ✅ | 5/5 | 2026-03-06 |
| Step 2 | 审计字段一致性与幂等门禁 | ✅ | 1/1 | 2026-03-06 |
| Step 3 | 导出任务与部署基准 API 测试基线 | ✅ | 5/5 | 2026-03-06 |
...
**验收标准**:
- [x] 缺失 `evidence_refs`、缺失 `baseline_job_id` 等错误可被稳定拒绝
- [x] 未确认请求不会触发任务创建
- [x] 确认后可创建任务并返回可追踪标识
...
**验收标准**:
- [x] 响应体、日志、存储模型中的审计字段同名同义
- [x] 重复提交不会产生不可控重复任务
- [x] 幂等冲突返回稳定错误结构或复用既有结果
...
**验收标准**:
- [x] 不支持 backend 与参数缺失可稳定拒绝
- [x] 导出状态查询可轮询并有一致返回结构
- [x] 部署基准查询契约稳定，失败映射 `EXPORT_FAILED`
...
### 实现检查清单
- [x] Red：工作流、审计、导出三类失败用例先行
- [x] Green：最小实现通过核心路径
- [x] Refactor：审计与参数校验抽象后回归全绿

### 质量标准
- [x] Proposal 与 Export 关键路径测试覆盖率达到阶段目标（建议 ≥ 80%）
- [x] 审计字段一致性与幂等行为可复测
- [x] 导出后端参数校验具备扩展性门禁
