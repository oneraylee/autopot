...
| 步骤 | 名称 | 状态 | 测试 | 完成日期 |
|------|------|------|------|----------|
| Step 1 | 任务编排 API 测试基线 | ✅ | 5/5 | 2026-03-06 |
| Step 2 | 日志/指标摘要 API 测试基线 | ✅ | 1/1 | 2026-03-06 |
| Step 3 | 分析产物查询契约与路径格式门禁 | ✅ | 4/4 | 2026-03-06 |
...
### 实现检查清单
- [x] Red：状态机、观测、产物三类失败用例先行
- [x] Green：最小实现通过核心路径
- [x] Refactor：契约不变且回归全绿

### 质量标准
- [x] 任务与产物查询关键路径测试覆盖率达到阶段目标（建议 ≥ 80%）
- [x] 状态机错误映射与产物错误语义可复测
- [x] 路径格式输出保持稳定并有测试门禁
